...
export function About() {
  ...
  return (
    <section id="about" className="section-padding bg-secondary/50 dark:bg-secondary/20 relative overflow-hidden py-20 md:py-28">
      <div className="absolute inset-0 bg-gradient-to-br from-accent/5 to-transparent pointer-events-none"></div>
      <div className="container-tight relative z-10">
        <div ref={elementRef} className={`flex flex-col md:flex-row gap-10 lg:gap-16 items-center ${isVisible ? 'animate-fade-in' : 'opacity-0'}`}>
          <div className="md:w-2/5 flex-shrink-0">
            <div className="relative">
              <div className={`w-56 h-56 md:w-72 md:h-72 lg:w-80 lg:h-80 rounded-full overflow-hidden border-4 border-accent/20 mx-auto transition-all duration-700 ${animateImage ? 'scale-100 shadow-xl' : 'scale-95 opacity-90'}`}>
                <img 
              ...
              <div className={`absolute -bottom-4 -right-4 w-20 h-20 md:w-24 md:h-24 bg-accent rounded-full flex items-center justify-center text-white font-bold transition-all duration-700 ${animateImage ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}>
                <span>+4 {t("about.experience")}</span>
              </div>
            </div>
          </div>
          <div className="md:w-3/5 px-4 md:px-0">
            ...
            <p className={`mb-8 transition-all duration-700 delay-300 ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-0'}`}>
              {t("about.paragraph3")}
            </p>
